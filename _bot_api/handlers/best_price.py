...
def best_deal(message, destination_id, count_print, photo_fl=False):
    ...
    min_cost_m = bot.send_message(
        message.chat.id, "Введите минимальную цену", reply_markup=markup_base
    )
    bot.register_next_step_handler(
        min_cost_m, get_min_cost, destination_id, count_print, photo_fl
    )
...
def get_min_cost(message, destination_id, count_print, photo_fl):
    ...
    max_cost_m = bot.send_message(
        message.chat.id, "Введите максимальную цену", reply_markup=markup_base
    )
    bot.register_next_step_handler(
        max_cost_m, get_max_cost, destination_id, count_print, photo_fl, min_cost
    )
...
def get_max_cost(message, destination_id, count_print, photo_fl, min_cost, max_cost=0):
    ...
    try:
        if int(min_cost) > int(max_cost):
            bot.send_message(
                message.chat.id, "Попробуйте еще раз", reply_markup=markup_base
            )
        ...
        bot.send_message(
            message.chat.id, "Попробуйте ещё раз", reply_markup=markup_base
        )
        best_deal(message, destination_id, count_print, photo_fl)

    min_star = bot.send_message(
        message.chat.id,
        "Введите минимальное количество звезд",
...
def get_min_star(message, destination_id, count_print, photo_fl, min_cost, max_cost):
    ...
    if min_star == "1":
        ...
        err_star = bot.send_message(
            message.chat.id, "Нужно целое число от 1 до 5", reply_markup=markup_base
        )
        get_max_cost(
            err_star, destination_id, count_print, photo_fl, min_cost, max_cost
        )
        return

    send_best(
        message, destination_id, count_print, photo_fl, min_cost, max_cost, stars_count
    )
...
def send_best(
    ...
    if check_command(message):
        return 1
    response = site_best_req(
        destination_id, count_print, min_cost, max_cost, stars_count
    )
