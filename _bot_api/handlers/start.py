...
def send_welcome(message):
    bot.send_message(
        message.chat.id, "Приветствую, я бот для поиска подходящих отелей."
    )
...
def get_id(message):
    ...
    if destination_id:
        bot.send_message(
            message.chat.id, "Поиск: \n{}".format(name), reply_markup=markup_base
        )
        count_message = bot.send_message(
            message.chat.id, "Сколько отелей отобразить? (до 24)"
        )
        ...
        bot.send_message(
            ...
            "попробуйте еще раз".format(message.text),
...
def get_count(message, destination_id):
    ...
    if message.text.isdigit() and (0 < int(message.text) < 26):
        ...
        count = bot.send_message(
            message.chat.id,
            "Сколько отелей отобразить? (до 24)",
            reply_markup=markup_base,
        )
...
def get_photo(message, destination_id, count_print):
    photo = bot.send_message(
        message.chat.id, "Отображать фотографии?", reply_markup=markup_bool
    )
    bot.register_next_step_handler(
        photo, func.menu.menu_button, destination_id, count_print
    )
